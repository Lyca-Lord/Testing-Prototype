...
public class Central : MonoBehaviour
{
    ...
    public LevelMapInfo levelMapInfo;
    ...
    [Header("Unit Event")]
    [HideInInspector] public UnityEvent<int> ComputerPlayerReinforceEvent;
    [HideInInspector] public UnityEvent<Units> UnitDieEvent;
    [HideInInspector] public UnityEvent UnitNumChangeEvent;

    public void StartGame(LevelInfo _levelInfo)
    {
        ...
        {
            levelMapInfo = levelInfo.mapInfo;

            yield return new WaitForEndOfFrame();
            MapManager.Instance.CreateMap(levelMapInfo);

            yield return new WaitForEndOfFrame();
            UnitManager.Instance.Register();

            yield return new WaitForEndOfFrame();
            DeployPhaseManager.Instance.StartDeployPhase(levelMapInfo);

            yield return new WaitForEndOfFrame();
            CardManager.Instance.SetUpCardAndBox();
        }
    }
    ...
    private void InitUnityEvents()
    {
        ...
        ComputerPlayerReinforceEvent ??= new UnityEvent<int>();
    }
    ...
    public void ClearAllUnityEvents()
    {
        ...
        ComputerPlayerReinforceEvent?.RemoveAllListeners();
    }
    ...
    public void UnitsNumberChange()
    {
        StartCoroutine(Enumerator());
        IEnumerator Enumerator()
        {
            yield return new WaitForEndOfFrame();
            UnitNumChangeEvent?.Invoke();
        }
    } // 直接唤起事件似乎会导致不同逻辑在同一帧顺序错误
}
