...
namespace Map
{
    public partial class MapManager
    {
        ...
        [SerializeField] private UnitInfo flagInfo; // 用于初始放置军营
    ...
    public partial class MapManager : MonoBehaviour, IInitialiazer
    {
        ...
        public void CreateMap(LevelMapInfo _levelMap)
        {
            if (cellList.Count > 0)
            {
            ...
            if (_levelMap.grids == null)
            {
                Debug.LogWarning("关卡地图数据不存在");
            }

            mapHeight = _levelMap.gridHeight;
            mapWidth = _levelMap.gridWidth;
            ...
            for (int i = 0; i < mapHeight; i++)
            {
                for (int j = 0; j < mapWidth; j++)
                {
                    ...
                    cell.Initialize(
                        index,
                        position * cell.CellSize,
                        new(i, j),
                        _levelMap.GetGrid(j, i)
                        );
                    cellList.Add(cell);
                }
            }

            // 2. 生成初始军营 (依赖 UnitManager)
            if (flagInfo != null && UnitManager.Instance != null)
            {
                // 生成玩家军营 (参数: info, isPlayer=true, isLocked=false, isCostFree=true)
                if (_levelMap.playerFlagPositions != null)
                {
                    foreach (var pos in _levelMap.playerFlagPositions)
                    {
                        UnitManager.Instance.SetUnit(flagInfo, true, false, true);
                        UnitManager.Instance.CreateUnit(new Vector2(pos.x, pos.y));
                    }
                }

                // 生成敌方军营 (参数: info, isPlayer=false, isLocked=false, isCostFree=true)
                if (_levelMap.enemyFlagPositions != null)
                {
                    foreach (var pos in _levelMap.enemyFlagPositions)
                    {
                        UnitManager.Instance.SetUnit(flagInfo, false, false, true);
                        UnitManager.Instance.CreateUnit(new Vector2(pos.x, pos.y));
                    }
                }
            }
            ...
            {
                Debug.LogWarning("无法生成初始军营：FlagInfo 或 UnitManager.Instance 为空。");
            }
        }
    ...
    public partial class MapManager
    {
        ...
        public void FindMovePath(Vector2 startPoint, Vector2 endPoint, int step, bool isPlayer, bool isTactic = false)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    int cost;
                    if (!isTactic)
                        cost = nextCell.type == 2 ? 2 : 1; // 不同地形的行动力消耗
                    else cost = 1; // 战术移动不考虑地形消耗
        ...
        public void HighLightMovePath(Vector2 startPoint, int step, bool isPlayer, bool isTactic = false)
        {
            DisableAllCell(); // 先禁用所有格子
            foreach (var i in cellList)
            {
                if (Distance(i.location, startPoint, "Manhattan") > step) continue;
                FindMovePath(startPoint, i.location, step, isPlayer, isTactic); // 每次只找一个点的最短路径
